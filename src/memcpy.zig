...
export fn kernelHighBandwidth(dst: *addrspace(.global) u32, src: *addrspace(.global) const u32) callconv(.nvptx_kernel) void {
...
export fn kernelLowLatency(dst: *addrspace(.global) u32, src: *addrspace(.global) const u32, n: u32) callconv(.nvptx_kernel) void {
    const index = MEMCPY_BLOCK_SIZE * nvptx.blockIdX() + nvptx.threadIdX();
    if (index < n)
        dst[index] = src[index];
}

export fn kernelTrailing(dst: *addrspace(.global) u32, src: *addrspace(.global) const u32, n: u32) callconv(.nvptx_kernel) void {
    const index = MEMCPY_BLOCK_SIZE * nvptx.blockIdX() + nvptx.threadIdX();
    if (index < n)
        dst[index] = src[index];
}

pub fn memcpy(comptime Type: type, dst: *align(@alignOf(u32)) Type, src: *align(@alignOf(u32)) const Type) !void {
